...
- Required reads:
  ...
  - prior verdicts via `verdict-parse(run-dir=<path>, type=<review-type>)`
...
- Conditional reads:
  ...
  - For `review_type: test-audit`:
    - Test paths via `test-path-resolve(run-dir=<path>)`.
    - Prod-code diff partition via `prod-diff-sha(base-sha=<sha>, head=HEAD, test-paths-file=<run-dir>/test-paths.txt)` for pin reference.
...
- Determine next `N` via `verdict-parse(run-dir=<path>, type=<type>)` max-revision read + increment.
