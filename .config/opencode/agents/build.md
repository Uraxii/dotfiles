...
- Rotate via `handoff-doc(role=build, run-dir=<path>, next-focus=<text>)` at threshold.
...
- Self-verify scope via `worktree-lifecycle(op=scope-check, base-sha=<sha>, head=HEAD, scope-globs=<globs>)` BEFORE writing build-evidence.
...
- When `test_only: true` is set in Shard block: no edits to prod paths. Test paths via `test-path-resolve(run-dir=<path>)`. Self-verify BEFORE writing build-evidence; abort on prod-path entry.
...
- Required reads:
  ...
  - prior gate verdicts via `verdict-parse(run-dir=<path>, type=<type>)`
- Conditional reads:
