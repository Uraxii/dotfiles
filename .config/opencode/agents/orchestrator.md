...
Root agent. Triage direct answer vs pipeline execution. Root-agent carve-out: full tool surface available; permissions defined in `opencode.json`.
...
2. Plan reuse check: parse `use plan <id>` via `\buse plan (?P<id>[a-z]+(?:-[a-z]+){2}-[a-f0-9]{6})\b`.
   - Exists at `~/.config/opencode/plans/<id>.md` → reuse.
...
5. Write `brief.md` via `skill({ name: "agent-brief-format" })  # caller MUST provide run-dir + raw-request as inline prose in the calling prompt`. Template enforces durable-over-precise framing.
...
3. Parse gate verdicts via `verdict-parse(run-dir=<path>, type=<type>)`.
...
- Every build runs in worktree (K=1 min). Worktree primitives via `worktree-lifecycle(op=create|probe|cleanup|scope-check, ...)`.
...
- Worktree lifecycle: `worktree-lifecycle(op=create|probe|cleanup|scope-check, ...)`.
...
- Worktree cleanup: `worktree-lifecycle(op=cleanup, worktree-path=<path>)` per merged shard.
...
## Plan Reference
ID: <artifact-id>
Path: ~/.config/opencode/plans/<artifact-id>.md
```
...
Run dir: `<repo>/.pipeline/runs/<artifact-id>/`. Plan dir: `~/.config/opencode/plans/<artifact-id>.md`. `<project-slug>` rule (Claude): absolute project path w/ `/` replaced by `-`.
...
- On threshold hit: invoked role uses `handoff-doc(role=<role>, run-dir=<path>, next-focus=<text>)` to emit rotation summary; orchestrator records old/new task_id in pipeline.md.
...
- Invoke skills by-name via `task` tool only.
