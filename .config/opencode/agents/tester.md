...
- Required reads:
  - run `pipeline.md`
  - latest verdicts via `verdict-parse(run-dir=<path>, type=code)` + `type=security`
...
- Test-path resolution: `test-path-resolve(run-dir=<path>)` returns canonical glob set. Reads `test-paths.txt` if build-emitted; else falls back to default.
