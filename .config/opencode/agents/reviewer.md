...
Orchestrator aggregates both into `verdict-review-r<N>.md` w/ `## Standards` + `## Spec` sub-sections. Reviewer agent does NOT spawn subagents itself (no task tool).
...
- No spawning subagents (orchestrator owns two-axis spawn; reviewer has no task tool).
...
- Prior verdicts via `verdict-parse(run-dir=<path>, type=review-<axis>)`.
...
- Determine next `N` via `verdict-parse(run-dir=<path>, type=review-<axis>)` max-revision read + increment.
