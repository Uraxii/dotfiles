...
~/.config/opencode/sessions/*/slack.json.

Usage (detached, via session_bind.py activate — do not run directly):
    uv run --script ~/.config/opencode/pipeline/comms/router.py
...
State (host-level, not stowed):
    ~/.config/opencode/comms-router/router.pid     — held flock + PID number
    ~/.config/opencode/comms-router/router.log     — daemon log
    ~/.config/opencode/comms-router/unrouted/      — unmatched events (audit)
    ~/.config/opencode/comms-router/run-index/     — per-run context for button routing
"""
