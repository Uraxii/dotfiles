...
~/.config/opencode/sessions/<sid>/slack.json and applies the B8 fix: sets
...
def session_state_path(session_id: str | None = None) -> Path:
    """Return Path to ~/.config/opencode/sessions/<sid>/slack.json. Does not check existence."""
...
def inbox_dir(session_id: str | None = None) -> Path:
    """Return Path to ~/.config/opencode/sessions/<sid>/inbox/. Does not create."""
    sid = _get_session_id(session_id)
    if not sid:
        raise ValueError(
            "session_id required: CLAUDE_CODE_SESSION_ID not set in environment"
...
def all_active_bindings() -> dict[str, dict[str, Any]]:
    """Glob ~/.config/opencode/sessions/*/slack.json; return {sid: parsed_json} for all active.
