...
```bash
uv run --script ~/.config/opencode/pipeline/session_bind.py activate
```
...
2. flock `~/.config/opencode/sessions/<sid>/.lock` (mutex; rejects concurrent activate).
3. Resolves Slack channel: `~/.config/opencode/pipeline/slack.env.local` `SLACK_CHANNEL` (global) → `<cwd>/.pipeline/pipeline.toml [slack].channel` (per-project override).
...
7. Else → posts root message `:hourglass_flowing_sand: *Session started* <sid-short> (cwd=~)` to channel. Captures `thread_ts`. Writes `~/.config/opencode/sessions/<sid>/slack.json` mode 600. Spawns host router (`comms/router.py`) if not already alive.
...
- Creates `~/.config/opencode/sessions/<sid>/` (mode 700) on first bind.
...
- One-shot reap of legacy `~/.config/opencode/slack-router/` daemon before spawning new daemon.

## Reverse

Use `slack-unbind` skill or `uv run --script ~/.config/opencode/pipeline/session_bind.py deactivate`.

## State

- `~/.config/opencode/sessions/<sid>/slack.json` — binding state, mode 600
- `~/.config/opencode/sessions/<sid>/inbox/<msg_ts>.json` — user replies routed by host router
- `~/.config/opencode/comms-router/router.{pid,log}` — host-level router process state (NOT per-session)
