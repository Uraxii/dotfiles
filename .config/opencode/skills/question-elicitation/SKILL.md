...
- `~/.config/opencode/pipeline/pipeline_ask.py` executable
- Active session binding: run `slack-bind` first. No binding → CLI exits 4 with clear message.
- `~/.config/opencode/pipeline/slack.env.local` w/ `SLACK_BOT_TOKEN`, `SLACK_APP_TOKEN`
...
```
Bash(
  command="python3 ~/.config/opencode/pipeline/pipeline_ask.py "
          "--run <artifact-id> "
          "--header 'Auth method' "
...
5. Router daemon (`comms/router.py`) handles `question_pick_<A..D>` button clicks.
...
- `~/.config/opencode/pipeline/comms/router.py` — host-bound router daemon (handles button clicks)
- `~/.config/opencode/pipeline/session_bind.py` — session binding helper (stdlib-only)
