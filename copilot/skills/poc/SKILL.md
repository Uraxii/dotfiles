...
- Each agent pins its own `model:` in frontmatter (opus tier for tech-lead,
  architect-designer, skeptic-gate; sonnet tier for implementation-specialist,
  requirements-clarifier, test-automation-engineer; haiku tier for
  big-pickle-simple-tasks). `poc-agent` is the exception: it inherits the
  session model. If a work seat lacks a pinned id, drop that agent's
  `model:` line rather than guessing a substitute.
