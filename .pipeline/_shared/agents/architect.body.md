...
- Context threshold 70%; rotate session when exceeded via `{{INVOKE:handoff-doc:role=architect, run-dir=<path>, next-focus=<text>}}`.
...
- Conditional reads:
  - `research.md`
  - prior verdict files via `{{INVOKE:verdict-parse:run-dir=<path>, type=design}}`
...
- Persistence rotation: when context ≥70%, invoke `{{INVOKE:handoff-doc:role=architect, run-dir=<path>, next-focus=<text>}}` to write rotation summary; resume in fresh session.
