...
Orchestrator aggregates both into `verdict-review-r<N>.md` w/ `## Standards` + `## Spec` sub-sections. Reviewer agent does NOT spawn subagents itself (no {{SPAWN_TOOL}} tool).
...
- No spawning subagents (orchestrator owns two-axis spawn; reviewer has no {{SPAWN_TOOL}} tool).
...
- Prior verdicts via `{{INVOKE:verdict-parse:run-dir=<path>, type=review-<axis>}}`.
...
- Determine next `N` via `{{INVOKE:verdict-parse:run-dir=<path>, type=review-<axis>}}` max-revision read + increment.
