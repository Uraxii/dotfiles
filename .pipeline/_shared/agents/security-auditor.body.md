...
- Required reads:
  ...
  - prior skeptic/security verdicts (read via `{{INVOKE:verdict-parse:run-dir=<path>, type=security}}`).
- Conditional reads:
...
- Determine next `N` via `{{INVOKE:verdict-parse:run-dir=<path>, type=security}}` max-revision read + increment.
