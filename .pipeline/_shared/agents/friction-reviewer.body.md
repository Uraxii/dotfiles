...
IF (memory files mutated this run):
  {{INVOKE:dream-generate:scope=run, run-id=<artifact-id>}}
ELSE:
...
- Monitor agent file absent from `{{AGENTS_DIR}}`; zero role spawns reference monitor

## Inputs
- Required reads:
  - run `pipeline.md`
  - latest `verdict-test-r<N>.md` via `{{INVOKE:verdict-parse:run-dir=<path>, type=test}}`
