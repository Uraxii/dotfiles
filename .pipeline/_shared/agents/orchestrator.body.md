...
Root agent. Triage direct answer vs pipeline execution. Root-agent carve-out: {{ROOT_TOOL_SURFACE}}
...
5. Write `brief.md` via `{{INVOKE:agent-brief-format:run-dir=<path>, raw-request=<user-text>}}`. Template enforces durable-over-precise framing.
...
3. Parse gate verdicts via `{{INVOKE:verdict-parse:run-dir=<path>, type=<type>}}`.
...
- Every build runs in worktree (K=1 min). Worktree primitives via `{{INVOKE:worktree-lifecycle:op=create|probe|cleanup|scope-check, ...}}`.
...
- Worktree lifecycle: `{{INVOKE:worktree-lifecycle:op=create|probe|cleanup|scope-check, ...}}`.
...
- Worktree cleanup: `{{INVOKE:worktree-lifecycle:op=cleanup, worktree-path=<path>}}` per merged shard.
...
Run dir: `<repo>/.pipeline/runs/<artifact-id>/`. Plan dir: `{{PLAN_PATH:<artifact-id>}}`. `<project-slug>` rule (Claude): absolute project path w/ `/` replaced by `-`.
...
- On threshold hit: invoked role uses `{{INVOKE:handoff-doc:role=<role>, run-dir=<path>, next-focus=<text>}}` to emit rotation summary; orchestrator records old/new task_id in pipeline.md.
...
- Invoke skills by-name via `{{SPAWN_TOOL}}` tool only.
