...
- **Designer** (`designer.md`) — read-only. Phases 0–3: req, data structures, interface
  contracts, TODO siting. Inline so user sees reasoning; subagent only for big read-only
  research.
- **Builder** (`builder.md`) — writes code. Phases 4–6: impl, dual-layer tests, invariants,
  behavioral runs, deviation log. Inline for small work; subagent for heavy impl.
- **skeptic** (`skeptic.md`) — independent challenge check. Spawn **fresh general-purpose
  subagent** w/ that prompt, phases 4 & 6 only. Separate spawn + clean context = independence.
  Builder never certifies own work.

Read `designer.md`, `builder.md`, `skeptic.md` before phases that use them.
...
- **deviation log** — every place forced to break from structs/interfaces/TODOs
