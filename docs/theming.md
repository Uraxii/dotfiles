...
- Nothing in this repo applies the dotfiles palette to KDE. Set the Qt color scheme and Plasma desktop theme yourself in System Settings; no login shell touches `kdeglobals` or `plasmarc`.
- GTK/XDG portal dark mode for Electron/GTK apps is not managed here either. Set it by hand if needed:
  ```bash
