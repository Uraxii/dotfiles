---
name: graphify
description: Use when a repository AGENTS.md instructs graphify usage or the user invokes /graphify. Provides safe project-level graphify query/update workflow; repo instructions decide when it runs.
version: 1.0.0
author: Hermes Agent
license: MIT
metadata:
  hermes:
    tags: [graphify, codebase-navigation, knowledge-graph, repository-context]
...
This skill explains how to use graphify safely. It does not decide whether graphify applies globally. The active repository's `AGENTS.md` / project instructions are the source of truth for when to use it.

## When to Use

Use this skill when:
- User invokes `/graphify`.
- Repo `AGENTS.md` says to use graphify for codebase questions.
...
1. Read the repo's `AGENTS.md` / local instructions first.
...
4. Do not bake graphify usage into Hermes personalities or global role prompts.
...
1. Running graphify because a personality says so. Wrong owner. Repo `AGENTS.md` owns graphify policy.
...
- [ ] Repo `AGENTS.md` checked before using graphify.
